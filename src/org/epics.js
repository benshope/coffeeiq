...
export const toggleMembershipFailedEpic = (action$, store) =>
  action$
    .filter(action => action.type === orgActions.TOGGLE_GROUP_MEMBERSHIP_FAILED)
    .map(({ payload }) =>
...
export const toggleMembershipSuccessEpic = (action$, store) =>
  action$
    .filter(
      action => action.type === orgActions.TOGGLE_GROUP_MEMBERSHIP_SUCCESS
    )
    .map(({ payload }) =>
...
export const deleteInviteFailedEpic = (action$, store) =>
  action$
    .filter(action => action.type === orgActions.DELETE_INVITE_FAILED)
    .map(({ payload }) =>
      notificationsActions.requestCreateErrorNotification({
...
export const deleteInviteSuccessEpic = (action$, store) =>
  action$
    .filter(action => action.type === orgActions.DELETE_INVITE_SUCCESS)
    .map(() =>
      notificationsActions.requestCreateSuccessNotification({
...
export const resendInviteFailedEpic = (action$, store) =>
  action$
    .filter(action => action.type === orgActions.RESEND_INVITE_FAILED)
    .map(({ payload }) =>
      notificationsActions.requestCreateErrorNotification({
...
export const resendInviteSuccessEpic = (action$, store) =>
  action$
    .filter(action => action.type === orgActions.RESEND_INVITE_SUCCESS)
    .map(() =>
      notificationsActions.requestCreateSuccessNotification({
...
export const createGroupFailedEpic = (action$, store) =>
  action$
    .filter(action => action.type === orgActionTypes.CREATE_GROUP_FAILED)
    .map(({ payload }) =>
      notificationsActions.requestCreateErrorNotification({
...
export const updateGroupEpic = (action$, store) =>
  action$
    .filter(action => action.type === orgActionTypes.UPDATE_GROUP)
    .flatMap(({ payload }) => {
      ...
      return new Promise((resolve, reject) =>
        firebaseDb.ref(`orgs/${orgId}/groups/${payload.key}`).update(
          ...
          error =>
            error
              ? reject(
                  orgActions.updateGroupFailed({ groupId: payload.key, error })
                )
...
export const updateGroupSuccessEpic = (action$, store) =>
  action$
    .filter(action => action.type === orgActionTypes.UPDATE_GROUP_SUCCESS)
    .map(() =>
      notificationsActions.requestCreateSuccessNotification({
...
export const updateGroupFailedEpic = (action$, store) =>
  action$
    .filter(action => action.type === orgActionTypes.UPDATE_GROUP_FAILED)
    .map(({ payload }) =>
      notificationsActions.requestCreateErrorNotification({
