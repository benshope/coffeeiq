...
export const signInEpic = action$ => {
  return action$
    ...
    .map(() => {
      ...
      let request = firebaseAuth.signInWithPopup(provider).then(authActions.signInSuccess, authActions.signInError);
...
export const signOutEpic = action$ => {
  return action$
    ...
    .map(() => {
      return Observable.fromPromise(firebaseAuth.signOut().then(authActions.signOutSuccess, authActions.signOutError));
    })
    .flatMap(x => [go("/sign-in"), x]);
};

export const signOutSuccessEpic = action$ => {
  return action$.filter(action => action.type === authActions.SIGN_OUT_SUCCESS).filter(() => {
    localStorage.clear();
